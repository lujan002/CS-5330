...
int main(int argc, char *argv[]) {
  ...
  cv::Mat rg_chromaticity_image;
  rg_chromaticity_image.create(src.size(), CV_8UC3);
  ...
  for( int i=0;i<src.rows;i++) {
    ...
    for(int j=0;j<src.cols;j++) {
      ...
      // OpenCV: [0]=B, [1]=G, [2]=R
      // saturate the values to the range [0, 255]
      rg_chromaticity_image.at<cv::Vec3b>(i, j)[2] = cv::saturate_cast<uchar>(r * 300);
      rg_chromaticity_image.at<cv::Vec3b>(i, j)[1] = cv::saturate_cast<uchar>(g * 300);
      rg_chromaticity_image.at<cv::Vec3b>(i, j)[0] = cv::saturate_cast<uchar>((1 - (r + g)) * 300);
    }
  }
  ...
  // scale the rg chromaticity image size to 10%
  cv::Mat resized_rg_chromaticity_image;
  cv::resize(rg_chromaticity_image, resized_rg_chromaticity_image, cv::Size(0, 0), 0.1, 0.1);
  cv::imshow("Source Image", resized_rg_chromaticity_image);

  // save the rg chromaticity image to a file
  cv::imwrite("../rg_chromaticity_image.png", rg_chromaticity_image);
  printf("RG chromaticity image saved to rg_chromaticity_image.png\n");

  // can save the histogram to a file if you want
  cv::imwrite("../histogram.png", dst);
  printf("Histogram saved to histogram.png\n");
