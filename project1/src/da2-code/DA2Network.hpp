...
public:
  ...
  DA2Network( const char *network_path ) {
    ...
    // This gives a bug when using onnxruntime-linux-x64-1.26.0
    this->session_ = new Ort::Session(env, network_path, Ort::SessionOptions{nullptr});

    // This works with onnxruntime-linux-x64-1.26.0, but is slow
  ...
  int run_network( cv::Mat &dst, const cv::Size &output_size ) {
    ...
    // Do not use a static Mat here: output H/W can change per run, and reusing the old buffer overflows.
