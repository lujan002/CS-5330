...
// Luke Jansen Notes:
// READS FEATURE CSV FILE: 
// read_image_data_csv — reads a feature CSV (filename in column 1, floats after).

// WRITES FEATURE VECTORS FOR EVERY IMAGE TO SAME CSV FILE
// append_image_data_csv — appends one row per call; 
// if you call it for every image in B with the same filename, you do build one CSV with 
// “feature vectors for every image” in that file.
